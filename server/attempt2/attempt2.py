...
f_outputs = K.function([combination_image_gen], outputs)
...
if K.image_data_format() == 'channels_first':
    x_style = np.random.uniform(0, 255, (1, 3, img_nrows, img_ncols)) - 128.
else:
    x_style = np.random.uniform(0, 255, (1, img_nrows, img_ncols, 3)) - 128.

if K.image_data_format() == 'channels_first':
    x_comb_gen = np.random.uniform(0, 255, (1, 3, img_nrows, img_ncols)) - 128.
else:
    x_comb_gen = np.random.uniform(0, 255, (1, img_nrows, img_ncols, 3)) - 128.


for i in range(iterations):
    print('Start of iteration', i)
    start_time = time.time()
    x_style, min_val, info = fmin_l_bfgs_b(evaluator.loss, x_style.flatten(),
                                           fprime=evaluator.grads, maxfun=20)
    print('Current loss value:', min_val, info)
    # save current generated image
    img = deprocess_image(x_style.copy())
